...
public class FTABBCucumberConcurrentRunner extends Runner {
    ...
    private List<ExecutorService> listaExecutor = new Vector<ExecutorService>();
    private List<String> listaAppConfig;
    private Class<?>[] listaClassesTesteParaExecucao;
    private List<ExecucaoPendente> listaExecucoesPendentes = new Vector<ExecucaoPendente>();
    private Map<Integer, Description> mapaDescriptionPorSequencialExecucaoTeste = new ConcurrentHashMap<Integer, Description>();
    private Method beforeClass;
    private Method afterClass;
    private List<CompletableFuture<Void>> listaFutures = new ArrayList<CompletableFuture<Void>>();
    private int novasTentativasAposFalharTeste = 0;
    ...
    public FTABBCucumberConcurrentRunner(Class<?> runner) throws InitializationError, IOException {
        ...
        this.listaAppConfig = new Vector<String>();
        for (String appConfig : appConfigAnnotation.value()) {
            this.listaAppConfig.add(appConfig);
        }
    ...
    public void run(RunNotifier notifier) {
        ...
        try {
            ...
            if (beforeClass != null) {
                beforeClass.invoke(null, new Object[0]);
            }
            ...
            for (int i = 0; i < this.listaFutures.size(); i++) {
                CompletableFuture<Void> future = this.listaFutures.get(i);
                future.get();
            }

            if (afterClass != null) {
                afterClass.invoke(null, new Object[0]);
            }
        } catch (Throwable e) {
    ...
    private String formatarMilissegundos(long milissegundos) {
        ...
        if (segundos > 0 || sb.length() > 0) {
            ...
            sb.append(segundos + " segundo(s)");
        }
        if (milissegundos > 0 || sb.length() > 0) {
            if (sb.length() > 0) {
                sb.append(", ");
            }
            sb.append(milissegundos + " milissegundo(s)");
        }
        return sb.toString();
    }
        ...
        return CompletableFuture.supplyAsync(new Supplier<Void>() {
            ...
            public Void get() {
                try {
                    ...
                    Method _metodoRunClasses = _classeJUnit.getDeclaredMethod("runClasses", new Class[]{Class[].class});
